...
func ExampleHandleCreateShortUrRequest() {
	f := facade.New(service.NewShortUrlService("http://localhost:8080", repository.NewMapKeyValueStorage()))
	h := handler.HandleCreateShortUrRequest(f)
...
func ExampleHandleRedirectRequest() {
	...
	h := handler.HandleRedirectRequest(facade.New(s))
...
func ExampleHandleCreateBatchShortUrRequest() {
	f := facade.New(service.NewShortUrlService("http://localhost:8080", repository.NewMapKeyValueStorage()))
	h := handler.HandleCreateBatchShortUrRequest(f)
...
func ExampleHandleGetUserURLsRequest() {
	...
	createHandler := auth(http.HandlerFunc(handler.HandleCreateShortUrRequest(f)))
	getHandler := auth(http.HandlerFunc(handler.HandleGetUserURLsRequest(f)))
...
func ExampleHandleDeleteUserURLsRequest() {
	...
	auth := middleware.AuthMiddleware("example-secret")
	createHandler := auth(http.HandlerFunc(handler.HandleCreateShortUrRequest(f)))
	deleteHandler := auth(http.HandlerFunc(handler.HandleDeleteUserURLsRequest(f)))
